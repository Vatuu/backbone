...
public class EventFactoryImpl {
    private static final List<ArrayBackendEvent<?>> ARRAY_BACKED_EVENTS = new ArrayList<>();

    private EventFactoryImpl() { }

    public static void invalidate() {
        ARRAY_BACKED_EVENTS.forEach(ArrayBackendEvent::update);
    }
    ...
    public static <T> Event<T> createArrayBacked(Class<? super T> type, T emptyInvoker, Function<T[], T> invokerFactory) {
        ArrayBackendEvent<T> event = new ArrayBackendEvent<>(type, emptyInvoker, invokerFactory);
